...
def harrison_ford
  ...
  # Actor.select(:id, :movie_id, :title).joins(:movies).where('name = \'Harrison Ford\' AND ord > 1')
  # Movie.select(:id, :title).joins(:actors).where('name = \'Harrison Ford\' AND ord > 1')
  Actor.select('movies.id, movies.title').joins(:movies).where("actors.name = 'Harrison Ford' AND castings.ord > 1")
...
def directed_by_one_of(them)
  ...
  # them = Actor.select(:id).where(name: them).pluck(:id)

  Movie.select(:id, :title).joins(:director).where(actors:{name: them})
  # Movie.select(:id, :title).joins(:actors).where(director_id: them).distinct
